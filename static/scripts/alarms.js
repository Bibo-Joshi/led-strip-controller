...
const addTimePicker = function (uid, hour, minute, type) {
    ...
    let picker = new AirDatepicker('#' + element_id, {
        ...
        buttons: [{
            content: "Abbruch", onClick: function (picker) {
                picker.hide()
            }
        }, {
            content: "Fertig", onClick: function (picker) {
                picker.hide();
                let hour = String(picker.selectedDates[0].getHours()).padStart(2, "0")
                let minute = String(picker.selectedDates[0].getMinutes()).padStart(2, "0")
                let effect_data = {};
